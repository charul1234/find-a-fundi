...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <a href="{{route('admin.seekers.create')}}" class="btn btn-primary btn-sm btn-icon-split float-right">
                <span class="icon text-white-50">
                  <i class="fas fa-plus"></i>
                </span>
                <span class="text">Add Seeker</span>
            </a>
            <h6 class="m-0 font-weight-bold text-primary">Seekers</h6>
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered" width="100%" cellspacing="0" id="seekers">
                    <thead>
                        <tr>
                          <th>Name</th>
                          <th>Email</th> 
...
function getUsers(){
    jQuery('#seekers').dataTable().fnDestroy();
    jQuery('#seekers tbody').empty();
    jQuery('#seekers').DataTable({
        processing: false,
        serverSide: true,
        ajax: {
            url: '{{ route('admin.seekers.getUsers') }}',
            method: 'POST'
        },
        lengthMenu: [
            [10, 25, 50, 100, -1],
